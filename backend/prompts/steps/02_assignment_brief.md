...
---BRIEF START---
ARTICLE TITLE: [exact H1 — max **60 characters**, **5–12 words**, benefit-led, not clickbait. Primary keyword appears **once** in the title (exact or near-exact). Count characters before submitting.]
ALTERNATIVE TITLES:
  1. [option — different angle; same 60-char / 5–12-word / keyword-once rules]
  2. [option — different framing, e.g. question format; same length rules]
...
KEYWORD USAGE RULE: [Primary keyword: once in H1 only. Each secondary keyword: at most once in the full article body or an H2 — no repetition.]
...
EXTERNAL AUTHORITY LINKS (REQUIRED unless editor Notes say otherwise):
  ...
  - Fix link quality: inline markdown only, working URLs from SERP, no bare URLs or broken placeholders.

IN-TEXT IMAGES (REQUIRED unless editor Notes say otherwise):
  - Plan **2–3** image placements across the article (after intro, mid-body, before FAQ or conclusion).
  - For each: section + suggested alt text (descriptive, keyword-aware where natural) + what the image should show.
  - Draft format: `![alt text](IMAGE: slug-or-description)` — not a stock photo dump at the end.
...
TONE: [describe the voice in 2–3 adjectives + one example sentence that captures it perfectly. Require **consistent** tone from intro through CTA — no mid-article shift in formality or perspective.]
...
RULES:
- Title must be **≤60 characters** and **5–12 words** — verify both before output
...
- Primary keyword appears **once** in ARTICLE TITLE; each secondary keyword used **at most once** in the brief's keyword plan
